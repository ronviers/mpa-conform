...
observation_window_sweep_v13 | 5 (Kernel/τ_obs — the camera category) | CLEAN (CORRECTED re-run) | one FIXED two-timescale EQUILIBRIUM signal (τ_f=1, τ_s=1000, a_s=0.6, X≈1) observed through 32 OBSERVATION WINDOWS (the CAMERA τ_obs, log-spaced 3→30000); the swept axis is the camera, NOT a substrate knob (the Cat-5 mark); at short windows the slow mode hasn't decayed → an apparent frozen plateau (~0.6) that MIMICS a glass q_EA; the plateau MELTS ~0.63→0 as the window opens, while the FDR slope ≈1 (X≈1) at EVERY window — now an EMERGENT FDT from two INDEPENDENT MC ensembles (C from a fluctuation ensemble, χ from a separate perturbation ensemble), not an imposed identity; the signal itself is fixed across runs; Δ→next: Cat 5 LANDED (5th category) — the camera-artifact foil to the intrinsic glass | BLIND MATCH (two-sided, corrected). Lands Cat 5 (Kernel/τ_obs) — the kernel pre-gate's own job (WORKFLOW §E; RFC-S §0.2 "τ_obs is the camera"): is an apparent character the SUBSTRATE or the OBSERVATION WINDOW? The blind answerer read the apparent frozen plateau as a CAMERA artifact, not intrinsic non-ergodicity, on TWO teeth: (a) the plateau MELTS to zero as the window opens (slow mode under-resolved, not stuck), and (b) X≈1 (slope-≈1 FDR locus) at every window (equilibrium / in balance — NOT a glass). It verified the SIGNAL is fixed (camera changes, not substrate), found the matched window, corrected the "permanently stuck / non-ergodic" worry. The camera-artifact FOIL to v4/v9/v10's intrinsic glass (there q_EA real, X<1, does NOT melt; here window artifact, X≈1, melts). Structural complement to every prior sweep (they moved a substrate knob; v13 moves the observer at fixed substrate). 32-box "camera movie" view. **METHOD BREAK + FIX (Ron caught it):** the FIRST v13 oracle set χ=(C0−C)/T (FDT imposed) → FDR locus the identity by construction → X=1 tautological (data-path-independence violation). REBUILT with C and χ as two INDEPENDENT MC measurements → FDT/X≈1 EMERGES within noise (corrected answerer read slope 0.997±0.014 and PARKED "whether X is exactly 1 — below resolution"). New standing rule: WORKFLOW §1 response-independence corollary. v4/v8/v9/v10/v12 annotated (their X legs imposed-not-tested; C-only findings stand). All 5 cage_edges avoided. First contact (foil to v4, no hard anchor). No KILL, not hollow. earned/.
```
...
a CAMERA artifact, not intrinsic, on two teeth: the plateau MELTS to zero as the window opens (~0.63→0),
and the FDR locus is slope ≈1 (X≈1) at every window. It verified the SIGNAL is fixed (camera, not
substrate, changes), found the matched window, corrected the "permanently stuck/non-ergodic" worry.**
Authored + glanced + posed + graded — AND then CORRECTED for an imposed-FDT flaw Ron caught (see the
correction arc below). Evolve done. v1/v3/v4/v5/v6/v7/v8/v9/v10/v11/v12/v13 in `earned/`, v2
documented-MISS. **`questions/` is now EMPTY** — no staged probe; the next vertical is unauthored.

**⚠ CORRECTION ARC (same session, 2026-05-26) — a real method break, fixed.** Ron flagged: "how can the
real data have a plateau... the in's and out's look like each other w.r.t. time, that doesn't seem
possible." Diagnosis: v13's FIRST oracle set the response χ=(C0−C)/T ANALYTICALLY (equilibrium FDT
**imposed**), so χ was the algebraic mirror of C → the FDR locus χ vs (C0−C) was the IDENTITY by
construction → the **X=1 reading was TAUTOLOGICAL** (a data-path-independence violation, WORKFLOW §1: two
independent measurements — fluctuation C and response χ — can't be exact mirrors). FIX (both halves, per
Ron): (1) **rebuilt v13** so C and χ are TWO INDEPENDENT Monte-Carlo ensembles (fluctuation vs a separate
step-perturbation), making FDT/X≈1 **emerge** within MC noise — re-posed + re-graded MATCH, the corrected
answerer read slope 0.997±0.014 and PARKED "whether X is exactly 1 — below resolution" (the tell the fix
worked); (2) **annotated v4/v8/v9/v10/v12** (their analytic oracles likewise imposed χ from C — their X
legs are imposed-not-tested; their C-only findings — band shape, two-step, melt, aging direction — stand);
(3) **new standing rule** WORKFLOW §1 *response-independence corollary* (never set χ=(C0−C)/T by fiat).
The earned/v13 record is now the corrected (independent-MC) version; git history holds the flawed first
version. **Lesson for future oracles: the response must be computed independently of the correlation, or
the FDT/X tooth is hollow.** The banach substrates (v3/v5/v6/v11) already did this right.
...
not drift). What remains is breadth (each needs a substrate/oracle BUILT) or polish:
   - **Breadth — the remaining GAP categories (each a build):** Cat 2 (Edge / reciprocal coupled-pair —
     ...
     **AND obey the new WORKFLOW §1 response-independence corollary:** if the oracle has a response/χ
     channel, compute χ from an INDEPENDENT measurement (e.g. a separate perturbation ensemble), never
     χ=(C0−C)/T by fiat — else the FDT/X tooth is tautological (the v13 break). For Cat 2 (a reciprocal
     coupled pair) this matters directly: its FDR/X reading must emerge, not be imposed.
