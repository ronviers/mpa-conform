...
NOT an inventor. It does exactly four things:
  ...
  4. refuse to emit if a framework token leaks into the blind half or the data
     header (code-side tripwire — blinding is enforced, not honor-system).

The sealed half of entry.md is never read out. The answerer-session works from the
emitted packet + the SANITIZED data file, never from entry.md or the raw .frozen.csv.
...
def blind_half(entry_text: str) -> str:
    idx = entry_text.find(SEALED_MARKER)
    if idx < 0:
        sys.exit(f"entry has no '{SEALED_MARKER}' divider — refusing to guess the blind/sealed split")
...
def main() -> None:
    ...
    print(f"  (sealed half of entry.md withheld; packet + data tripwires passed)")
