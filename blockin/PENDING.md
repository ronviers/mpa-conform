...
| 2026-05-25 | `blockin/questions/three_species_cycle_noise_sweep_v5/` (`entry.md`, freeze, `data/*.frozen.csv`) | authored-not-run vertical | v5 noise sweep (spends v3's owed vector) authored + freeze-computed; key cleared the human glance EXCEPT the winding 2nd-moment is estimator-noisy → caveat filed (`docs/deferred-for-auditor.md` Entry 2). NOT yet posed/blind-run — the rate verdict (the MATCH) is independent of the noisy spread, so it's runnable as-is; parked pending the run-now-vs-improve-estimator-first call | v5 arc (orchestrator) | pass runs (pose→blind→grade→`earned/`) or v5 retired |
