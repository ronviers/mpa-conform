...
| 1 | 2026-05-15 | Inversion engine relocated | Ported the auditor's `inversion-engine.js` + `gfdr-model.js` + `ensemble-locus.js` + `phase-locking-model.js` + the universal two-mode kernel (cdv1 §Universal two-mode kernel, Lamb closure) to Python under `conformer/compute/`. Pure numpy (no native build chain). Two-stage fit operational: Stage 1 analytical grid search, Stage 2 ensemble refine with cooperative-band divergence guard + analytical fallback. Activity-scroll CLI at `conformer/cli.py` (`python -m conformer.cli invert <bundle>` / `invert-all`). 5/5 parity tests pass; full library inverts 60/60 with 0 errors in <60s. | First load-bearing move toward the SUITE_BLOCK_IN rebalance — compute lives in conform, viewers consume. The fit results currently saturate at the analytical model's tau range for surface-code-qec and the glass band (substrate-conditional tau-rescaling is the known follow-up — flagged on the auditor side in the M6 session log; not introduced by this port). v0.2 schema bump (embed real fit + predicted_locus + audit_delta) and integration into the curator walk-library pipeline are the next obvious moves; this session deliberately keeps inversion as a separate CLI step so the relocation is testable in isolation. |
| 2 | 2026-05-17 | Three-way comparison display | New `conformer/compare/banach_overlay.py` + `compare` / `compare-all` CLI subcommands. Two-panel C(τ) + χ(τ) PNG per bundle showing empirical (markers + SEM), predicted (framework analytical at fitted chit, recomputed in dimensionless τ frame so the model has its full dynamic range), and Banach (protocol-matched: single canonical state per measurement window at `chit_ν = chit_0·exp(-ν)` with `ν = tau_obs/tau_scale`, observable swept within window). 22/22 ck-glassy cells rendered into gitignored `output/comparisons/`. | Two within-session iterations driven by looking at the PNG: first reading swept canonical state across the lag axis, user spotted that this conflated substrate-side time with canonical-side depth; second reading fixed `ν` to the bundle's `tau_obs` and swept the observable within the window. New `feedback_single_move_design` memory captures the iteration discipline (ship-one-move + render PNG + look together + plan next move from observation). New `project_mpa_conform_comparison_lens` memory captures the parallax + channel-richness + adapting-not-overfitting framing that came out of the second iteration. Single next move recorded in `docs/next-session-handoff.md`. |
